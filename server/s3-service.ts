...
export class S3Service {
  ...
  constructor() {
    this.bucketName = 'game.thynxai.cloud';
  }

  /**
   * Initialize S3 client with credentials from AWS Secrets Manager
   */
  ...
  private async doInitialize(): Promise<void> {
    try {
      const credentials = await secretsService.getAWSCredentials();
      ...
      console.error('Failed to initialize S3Service with secrets manager:', error);
      throw new Error('AWS credentials could not be retrieved from Secrets Manager');
    }
  }
  ...
  async deleteFile(key: string): Promise<boolean> {
    try {
      ...
      await this.s3Client!.send(command);
      console.log(`S3 delete command sent successfully for: ${key}`);
      return true;
    } catch (error) {
