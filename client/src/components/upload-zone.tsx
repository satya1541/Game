...
import { uploadFiles, deleteFile, updateFile } from "@/lib/api";
...
export default function UploadZone() {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (files: FileList) => {
      ...
      const { response, xhr } = await uploadFiles(files, undefined, (progressInfo) => {
    ...
    onSuccess: (data) => {
      ...
      setPopup({
        ...
        message: `${data.files.length} file(s) uploaded successfully`
      });
  ...
  const processFiles = (files: FileList) => {
    ...
    if (invalid.length > 0) {
      ...
      // Convert valid files to FileList for consistent handling
      const fileList = new DataTransfer();
      valid.forEach(file => fileList.items.add(file));
      uploadMutation.mutate(fileList.files);
    }
