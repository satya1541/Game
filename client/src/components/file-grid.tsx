...
const getCategoryColor = (category: string) => {
  switch (category) {
    case 'apks': return 'text-accent';
    case 'softwares': return 'text-primary';
    case 'scripts': return 'text-destructive';
    case 'archives': return 'text-secondary';
    case 'configs': return 'text-primary';
...
export default function FileGrid({ category, searchQuery, sortBy, showActions = 'download', currentUserId, viewMode = 'grid', selectedFolder, onFolderSelect }: FileGridProps) {
  ...
  const [confirmDialog, setConfirmDialog] = useState({
    isOpen: false,
    type: 'delete' as 'delete' | 'edit' | 'confirm',
    title: '',
    message: '',
    fileName: '',
    fileId: '',
    ...
    if (showActions === 'download') {
      return (
        <Button
          ...
          className="bg-gradient-to-r from-primary to-accent text-background px-4 py-2 rounded-lg font-semibold hover:shadow-lg hover:shadow-primary/25 transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
  ...
  if (!selectedFolder && !searchQuery && folders.length > 0) {
    return (
      <section className="py-8 px-4 sm:px-6 lg:px-8" id="folders">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {folders.map((folder: any) => (
              <div
                key={folder.name}
                className="folder-card rounded-xl p-6 cursor-pointer group relative overflow-hidden hover:scale-105 transition-all duration-200 shadow-lg shadow-black/30 hover:shadow-xl hover:shadow-black/40"
                onClick={() => onFolderSelect && onFolderSelect(folder.name)}
                data-testid={`folder-${folder.name}`}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-accent/30 rounded-xl"></div>
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-primary/30 to-accent/40 flex items-center justify-center">
                      <Archive className="text-2xl text-primary" />
                    </div>
          ...
          {files.map((file: any) => {
            ...
            return (
              <div 
                key={file.id} 
                className={cn(
                  "file-card rounded-xl p-6 group relative overflow-hidden",
                  viewMode === 'grid' ? "flex flex-col h-full" : "flex flex-row items-center space-x-6"
                )} 
                data-testid={`file-card-${file.id}`}
              >
                {/* Transparent gradient overlay like folder cards */}
                <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-accent/30 rounded-xl"></div>
                ...
                {viewMode === 'grid' ? (
                  <>
                    <div className="flex items-center justify-between mb-4">
                      <div className={cn(
                        ...
                        file.category === 'apks' && "bg-gradient-to-br from-accent/20 to-accent/40",
                        file.category === 'softwares' && "bg-gradient-to-br from-primary/20 to-primary/40",
                        file.category === 'scripts' && "bg-gradient-to-br from-destructive/20 to-destructive/40",
                        file.category === 'archives' && "bg-gradient-to-br from-secondary/20 to-secondary/40",
                        file.category === 'configs' && "bg-gradient-to-br from-primary/20 to-accent/20"
                      )}>
                        <Icon className={cn("text-2xl", colorClass)} />
                      </div>
                      <span className={cn(
                        "text-xs px-2 py-1 rounded-full font-medium",
                        file.category === 'apks' && "bg-accent/10 text-accent",
                        file.category === 'softwares' && "bg-primary/10 text-primary",
                        file.category === 'scripts' && "bg-destructive/10 text-destructive",
                        file.category === 'archives' && "bg-secondary/10 text-secondary",
                        file.category === 'configs' && "bg-primary/10 text-primary"
                      )}>
                  ...
                  <>
                    ...
                    <div className={cn(
                      ...
                      <Icon className={cn("text-3xl", colorClass)} />
                    </div>
