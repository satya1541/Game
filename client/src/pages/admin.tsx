      ...
      <main className="relative z-10 flex-grow">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 glass-light rounded-lg flex items-center justify-center border border-destructive/30">
              <Shield className="w-5 h-5 text-destructive" />
            </div>
            <div>
              <h1 className="text-3xl font-gaming font-bold bg-gradient-to-r from-destructive to-[#de5c5c] bg-clip-text text-transparent">
                Admin Dashboard
              </h1>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2">
                {adminStats?.overview?.s3StorageInfo?.optimizations?.map((optimization: string, index: number) => (
                  <div key={index} className="flex items-center space-x-3 p-3 glass-light rounded-lg border border-primary/20">
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {(adminStats?.overview?.categoryStats || stats?.categoryStats) && Object.entries(adminStats?.overview?.categoryStats || stats.categoryStats).map(([category, count]) => (
                  <div key={category} className="flex items-center justify-between p-3 glass-light rounded-lg border border-primary/20">
                    <div className="flex items-center space-x-3">
            ...
            <CardContent>
              <div className="space-y-4">
                {allFiles?.files?.slice(0, 10).map((file: any, index: number) => (
                  <div key={file.id} className="flex items-center justify-between p-3 glass-light rounded-lg border border-primary/20">
                    <div className="flex items-center space-x-3">
              ...
              <CardContent>
                <div className="space-y-3">
                  {adminStats.recentDownloads.slice(0, 10).map((download: any, index: number) => (
                    <div key={index} className="flex items-center justify-between p-3 glass-light rounded-lg border border-primary/20">
                      <div className="flex items-center space-x-3">
